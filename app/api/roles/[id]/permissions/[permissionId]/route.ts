...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: "Permission removed from role" }, { status: 200 })
  } catch (error) {
    if ((error as { code?: string })?.code === "P2025") {
      return NextResponse.json({ error: "Role or permission mapping not found" }, { status: 404 })
    }
